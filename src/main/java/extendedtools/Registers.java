...
public class Registers {
    ...
    public static final DeferredRegister<CreativeModeTab> CREATIVE_TABS = DeferredRegister
	    .create(Registries.CREATIVE_MODE_TAB, References.ID);
    ...
    public static final DeferredRegister<ArmorMaterial> ARMOR_MATERIALS = DeferredRegister
	    .create(Registries.ARMOR_MATERIAL, References.ID);
    ...
    public static final DeferredHolder<ArmorMaterial, ArmorMaterial> STEEL = register("steel",
	    ExtendedArmorMaterials.STEEL, SoundEvents.ARMOR_EQUIP_IRON);
    public static final DeferredHolder<ArmorMaterial, ArmorMaterial> BRONZE = register("bronze",
	    ExtendedArmorMaterials.BRONZE, SoundEvents.ARMOR_EQUIP_IRON);
    public static final DeferredHolder<ArmorMaterial, ArmorMaterial> TIN = register("tin", ExtendedArmorMaterials.TIN,
	    SoundEvents.ARMOR_EQUIP_IRON);
    public static final DeferredHolder<ArmorMaterial, ArmorMaterial> COPPER = register("copper",
	    ExtendedArmorMaterials.COPPER, SoundEvents.ARMOR_EQUIP_IRON);
    public static final DeferredHolder<ArmorMaterial, ArmorMaterial> LEAD = register("lead",
	    ExtendedArmorMaterials.LEAD, SoundEvents.ARMOR_EQUIP_IRON);
    public static final DeferredHolder<ArmorMaterial, ArmorMaterial> VANADIUM = register("vanadium",
	    ExtendedArmorMaterials.VANADIUM, SoundEvents.ARMOR_EQUIP_IRON);
    public static final DeferredHolder<ArmorMaterial, ArmorMaterial> SILVER = register("silver",
	    ExtendedArmorMaterials.SILVER, SoundEvents.ARMOR_EQUIP_IRON);
    public static final DeferredHolder<ArmorMaterial, ArmorMaterial> TITANIUM = register("titanium",
	    ExtendedArmorMaterials.TITANIUM, SoundEvents.ARMOR_EQUIP_IRON);
	...
	for (Tiers tier : Tiers.values()) {
	    DeferredHolder<Item, PaxelItem> obj = Registers.ITEMS.register("paxel" + tier.name().toLowerCase(),
		    () -> new PaxelItem(tier,
			    (tier == Tiers.NETHERITE ? new Properties().stacksTo(1).fireResistant()
				    : new Properties().stacksTo(1))
				    .attributes(AxeItem.createAttributes(tier, 7f, -3.5f))));
	...
	for (ExtendedArmorMaterials material : ExtendedArmorMaterials.values()) {
		...
		ITEMS.register(ARMOR_MAP.get(type.getName()) + material.getName(),
			() -> new ArmorItem(materialMap.get(material), type,
				new Properties().stacksTo(1).durability(material.getDurabilityForType(type))));
	    }
	}

	MAIN = CREATIVE_TABS.register("itemgroupextendedtools",
		() -> CreativeModeTab.builder().title(Component.translatable("itemGroup.itemgroupextendedtools"))
			.icon(() -> new ItemStack(ICON.get())).build());

    }

    private static DeferredHolder<ArmorMaterial, ArmorMaterial> register(String name, ExtendedArmorMaterials material,
	    Holder<SoundEvent> sound) {
	return ARMOR_MATERIALS.register(name,
		() -> new ArmorMaterial(material.map, material.enchantability, sound,
			() -> Ingredient.of(material.repairIng), List.of(material.layer), material.toughness,
			material.knockbackRes));
    }
